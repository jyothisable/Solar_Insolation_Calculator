...
def findSolarInsolation(day, time):
    ...
    # input cloud data
    gs.run_command('r.in.gdal', input='data/input_clouds/3DIMG_'+day+'JAN2020_' +
                   time_UTC+'_L2B_CMK_CMK.tif', output='cloud', overwrite=True, flags='o')
    # cleaning cloud data
    gs.run_command('r.mapcalc.simple', a='cloud',
                   expression='result =(1- A*(A<=1)*0.5)', output='cloud_cf', overwrite=True)
    ...
    gs.run_command('r.sun', elevation='DEM', horizon_basename='horangle',
                   ...
                   glob_rad='global_rad', day=day, time=solar_time, nprocs=6, linke_value=7,
